...
namespace CafeLibrary
{
    public class BFRES : FileEditor, IFileFormat, IDisposable
    {
        ...
        public Syroot.NintenTools.NSW.Bfres.ResFile ResFile { get; set; }

        public bool UpdateTransformedVertices = false;

        public void Load(Stream stream)
        {
            ResFile = new Syroot.NintenTools.NSW.Bfres.ResFile(stream);
            Reload();
        }
        ...
        public void TryInsertParamKey(string material, Syroot.NintenTools.NSW.Bfres.ShaderParam param)
        {
            var anim = this.Workspace.GetActiveAnimation();
        ...
        {
            ResFile = new Syroot.NintenTools.NSW.Bfres.ResFile();
        ...
        private void Reload()
        {
            ...
            if (true)
            {
                ...
                BntxFile Texs = new BntxFile(new MemoryStream(data));
                bntx = Texs;
            }
        ...
        private string GetSampler(string textureName)
        {
            foreach (var model in ResFile.Models) {
                foreach (var mat in model.Materials) {
                    ...
                    {
                        if (mat.TextureRefs[i] == textureName)
