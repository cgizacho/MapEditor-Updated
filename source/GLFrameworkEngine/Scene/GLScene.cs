...
namespace GLFrameworkEngine
{
    public partial class GLScene
    {
        ...
        private List<ITransformableObject> FindSelectableObjects(IEnumerable<object> objects)
        {
            ...
            {
                IDrawable mObj = (IDrawable)obj;

                if (!mObj.IsVisible)
                    continue;

                if (mObj is IEditModeObject && ((IEditModeObject)mObj).EditMode)
                {
                    foreach (var ob in ((IEditModeObject)mObj).Selectables)
                        transformables.Add(ob);
                }

                if (mObj is ISelectableContainer) {
                    foreach (var ob in ((ISelectableContainer)mObj).Selectables)
                        transformables.Add(ob);
                }
                if (mObj is ITransformableObject)
                    transformables.Add((ITransformableObject)mObj);
            }
